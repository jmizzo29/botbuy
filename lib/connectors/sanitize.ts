...
export function sanitizeAuditMetadata(
  ...
  for (const [key, value] of Object.entries(metadata ?? {})) {
    ...
    if (
      typeof value === "string" &&
      (/bearer\s+|sk_|rk_|pk_|whsec_|dop_v1_|shpat_|shpua_|shpca_|api[_-]?key/i.test(value) || looksLikePan(value))
    ) {
      continue;
    }
    out[key] = value;
...
export function assertNoSecretsLogged(value: unknown) {
  ...
  if (
    /BOTBUY_VAULT_KEY|ApiKey=|AuthToken=|Bearer\s+[A-Za-z0-9._-]+|(?:sk|rk|pk|whsec|dop_v1|shpat|shpua|shpca)_[A-Za-z0-9]+/i.test(
      blob,
    )
  ) {
