...
When this surface ships, this file is the Settings → Connected accounts IA SoT. IA here wins if craft and IA disagree.
...
Connect is API user + API key (official API). Never a Namecheap password. Never an HTML login farm.

## Twilio

- Prefer **OAuth** when Twilio allows it.
- **API key connect is OK for this POC** with disclosure: keys are encrypted, never logged, and this is not a live public connector.
