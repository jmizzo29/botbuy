...
1. `/intent` — starters include **Anything / Car / House / Software / Domain / Official catalog**. Start search for a car or house (no `qa-needs-you`). My deals stays **Searching** with `category=vehicle|property` · `accepted=true` · `live:false` · no invented results. Not rejected. Not mapped to Shopify.
