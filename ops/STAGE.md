...
1. `/intent` — starters include **Anything / Car / House / Software / Domain / Official catalog / Droplet**. Start search for a car, house, or a consumer product (no `qa-needs-you`, no HTTP JSON Preview keys). My deals stays **Searching** with `category=vehicle|property|product|general` · `accepted=true` · `live:false` · no invented results. Not rejected. Not mapped to Shopify or DigitalOcean.
...
2b. Droplet starter (or intent text with “DigitalOcean droplet”) → **Connector search · digitalocean · stub · keysConfigured=false**. Empty stub stays Searching. Not Namecheap. Not Shopify.
...
4. Car/House (or Anything) + **`qa-needs-you`** → Needs you → Approve → Buying. Deal detail **Prepare Checkout Session**. Without `BOTBUY_STRIPE_*`: `keysConfigured=false` · `prepared=false` · `sessionCreated=false` · `charged=false` · `live:false`. `/vault` shows the same key smoke. Auto-approve OFF. When `HTTP_JSON_BASE_URL` is on Preview, a car/house/product intent (no fixture) attempts the HTTP JSON MCP catalog instead of an empty stub — still `live:false`, no invented prices, still not Shopify.

**CPO/CHO connector key-readiness smoke (MCP-first · no live spend):**
1. Sign in on [stage](https://stage.botbuyer.ai) → **Settings → Connected accounts** (`/settings/connected-accounts`). Honesty strip shows `live=false` · `spend=false` · `keysConfigured=false` (until Preview env or a vault connect) · `mutationsLiveEnabled=false` · Search only.
...
3. Click **Read-only smoke** (or `POST /api/connectors/smoke` with `{ "provider": "digitalocean" }`). Expect `spend=false` · `tool=search` · `result=stub` · `keysConfigured=false` · `live=false` when secrets are missing. Domain / phone / droplet / official catalog intents without keys stay Searching.
