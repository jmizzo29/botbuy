...
John must add `NEXT_PUBLIC_CLERK_PUBLISHABLE_KEY`, `CLERK_SECRET_KEY`, and `DATABASE_URL` in Vercel. Optional: `CLERK_WEBHOOK_SECRET` + Clerk webhook → `/api/webhooks/clerk`. Apply `drizzle/0001_clerk_user_id.sql` or `npm run db:push`. With `DATABASE_URL`, Start search persists engine deals / events / usage / intents in Neon so My deals survives Vercel isolates. The `bb_engine_journal` cookie is a size-capped fallback only.
